...
class GridModel(object):
	...
	def __init__(self, mesh, out_dir='./output/', verbose=True):
		...
		dl.parameters['form_compiler']['no-evaluate_basis_derivatives'] = False
		...
		self.MPI_rank    = dl.MPI.rank(dl.MPI.comm_world)   # MPI process ID #
	...
	def initialize_variables(self):
		...
		* ``self.m0``         -- inital mass :math:`m_i^0`
		...
		self.assf_int_x = dl.FunctionAssigner(self.f_int_x.function_space(), self.Q)
		self.assf_int_y = dl.FunctionAssigner(self.f_int_y.function_space(), self.Q)
		self.assf_int_z = dl.FunctionAssigner(self.f_int_z.function_space(), self.Q)
